...
public class RFX4_TransformMotion : MonoBehaviour
{
    ...
    void UpdateWorldPosition()
    {
        ...
        if (!isCollided && Physics.Raycast(t.position, t.forward, out hit, 10, CollidesWith))
        {
            ...
            {
                isCollided = true;
                Enemy e = GameObject.Find("EnemySpawn").GetComponent<Enemy>();
                Player p = GameObject.Find("PlayerSpawn").GetComponent<Player>();
                if (CollidesWith == LayerMask.GetMask("HitCollider"))
                {
                  e.TakeDamage(skill.baseDamage + p.stats[0].baseValue);
                }
                else if (CollidesWith == LayerMask.GetMask("PlayerHitCollider"))
                {
                  p.TakeDamage(skill.baseDamage + e.stats[0].baseValue);
                }
